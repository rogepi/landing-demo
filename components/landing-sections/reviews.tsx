...
export function Reviews() {
  ...
  return (
    <motion.section className="mx-auto mb-20 text-center">
      <motion.header className="text-3xl font-bold md:text-5xl">
        Loora is great for
        <motion.span className="text-blue-600"> freelancers</motion.span>
      </motion.header>

      <motion.div className="mx-auto my-12 flex w-full gap-10 text-pink-500">
        <Ticker
          duration={30}
        ...
        >
          {mockData.map((item, index) => (
            <motion.div
              key={index}
              className="mx-4 flex h-[370px] w-[300px] flex-initial shrink-0 flex-col justify-between overflow-hidden rounded-xl border-[3px] border-pink-500 p-10 text-left"
            >
              <div>
